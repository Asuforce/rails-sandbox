...
RSpec.describe TodosController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    it 'returns a success respone' do
      ...
      expect(response_json.first['id']).to todo.id
    end
  ...
  describe 'POST #create' do
    context 'with valid params' do
      ...
      it 'renders a JSON response with the new todo' do
        ...
        expect(response_json.location).to eq(todo_url(Todo.last))
      end

      it 'unauth without CSRF' do
        request.cookies[JWTSessions.access_cookie] = @tokens[:access]
        post :create, parms: { todo: valid_attributes }
        expect(response).to have_http_status(401)
      end
    end
  ...
  describe 'PUT #update' do
    ...
    context 'with invalid params' do
      it 'renders a JSON response with errors for the todo' do
        request.cookies[JWTSessions.access_cookie] = @tokens[:access]
        request.headers[JWTSessions.csrf_header] = @tokens[:csrf]
        post :update, params: { id: todo.to_param, todo: invalid_attributes }
